...
객체는 그안에 멤버변수(property)와 매서드(function)가 내장되어 있음을 의미

그러므로 일반적으로 생성하는 문자열 변수 안에는 매서드와 프로퍼티가 자동으로 내장된다

const foo = "Hello World";
foo.매서드()

문자열 객체에 내장된 매서드들은 변수가 담고있는 내용을 가공하거나 특정 내용을 추출하는 기능을 갖는다
...
//기능을 확인하기 위한 문자열의 선언
const msg = 'Life is too short, You neew Javascript.';
console.log('문자열: ' + msg);
...
console.log('2번째 글자:' + str2nd);

//모든 문자열은 그 자체로 배열처럼 취급될 수 있다.
console.log('2번째 글자:' + msg[2]);

//파라미터로 전달된 내용이 처음 나타나는 위치를 리턴한다
const p1 = msg.indexOf('f');
console.log('f가 처음 나타나는 위치:' + p1);

//찾지 못할 경우 -1을 반환함
console.log('z가 처음 나타나는 위치: ' + msg.indexOf('z'));

//단어나 문장으로 검색할 경우 일치하는 내용이 시작되는 첫 글자의 위치를 반환
console.log('short가 처음 나타나는 위치:' + msg.indexOf('short'));

//indexOf에 파라미터가 두개인경우
//두번째 숫자값은 첫번째 파라미터의 글자를 찾기 시작하는 위치를 의미한다
const p2 = msg.indexOf('i');
const p3 = msg.indexOf('i', p2 + 1);
console.log('i가 첫번째로 나타나는 위치: ' + p2);
console.log('i가 첫번째로 나타나는 위치: ' + p3);

//파라미터로 전달된 글자가 마지막으로 나타나는 위치를 리턴한다
//단 이 위치를 문자열의 끝에서 부터 세지않고 문자열의 처음부터센다
//찾지 못할 경우 -1을 반환
...
console.log('모든글자의 대문자로 변환:' + up);

//모든 글자를 소문자로 변환한다
const low = msg.toLowerCase();
console.log('모든글자의 소문자로 변환:' + low);
