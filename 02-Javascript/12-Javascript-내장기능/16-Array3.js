...
arr.forEach((v, i) => {
  if (i == 3) {
    console.log(' ~~~반복중단!!!');
    //콜백함수 역시 함수이므로 break을 사용하여 중단할 수 없다
    //break;
    //forEach의 콜백함수에서 반복을 중단하고자 return을 사용할경우 현재 동작중인 콜백만 종료된뿐 전체 반복에는 영향이없다
    return;
  }
...
//화살표 함수는 처리로직이 한 줄인 경우 {}와 ; return키워드 생략가능함
const hello = arr.map((v, i) => v + 1);

//-> Array(5) [11,21,31,41,51]
console.log(hello);
