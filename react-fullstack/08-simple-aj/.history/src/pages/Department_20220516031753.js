...
const Department = () => {
    ...
    setTimeout(() => {
      (async () => {
        ...
        try {
          ...
          json = <response className="data"></response>;
        } catch (err) {
          console.error(err);
  ...
  //성능 최적화를 위해 useCallback() wjrdydgka
  ...
  React.useEffect(() => {
    if (dropId > -1) {
      //id가 일치하지않는 항목만 filter로 걸러내어 상태값 갱신함
  ...
  const onUpdateSubmit = React.useCallback((e) => {
    ...
    //form요소 내의 input요소들을 name속성값으로 접근하여 입력값을 얻음
    ...
    (async () => {
      ...
      if (json !== null) {
        //함수형 업데이트
        //--> 콜백 함수의 파라미터로 상태값의 복사본이 전달되므로 이값을 직접 수정해도 된다.
        setDepartment((department) => {
          //원본 상태값과 비교하여 숴정된 항목의 배열 인뎃스를 찾는다
          const find = department.findIndex((v, i) => v.id === json.id);
          //원본 상태갓의 해당 인덱스 번호 (find)위치부터 1개를 ajax로 반환받은 수정결과 데이터로 교체한다
