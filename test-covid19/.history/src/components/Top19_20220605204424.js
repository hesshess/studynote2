...
const Top19 = memo(({ sendToApp }) => {
  ...
  React.useEffect(() => {
    dispatch(getCovid(date_gte : (date1 + 'T00:00:00Z') , date_lte: (date2 + 'T00:00:00Z')));
  }, [dispatch]);
  ...
  const onSelect1 = (e) => {
    ...
    setDisable(!disable);
    // let data1 = data.filter((v) => v.date === date1 + 'T00:00:00Z');
    // console.log(JSON.stringify(data1));
  };
  const onSelect2 = (e) => {
    setDate2(e.target.value);
    // let data2 = data.filter((v) => v.date === date2 + 'T00:00:00Z');
