  ...
  return (
    <div>
...

      {
        <>
          <nav>
            <MenuLink to={`/confirmed?date_gte=${dateId1}&date_lte=${dateId2}`}>
              일일확진자
            </MenuLink>
            <MenuLink
              to={`/confirmed_acc?date_gte=${dateId1}&date_lte=${dateId2}`}
            >
              누적확진자
            </MenuLink>
            <MenuLink to={`/active?date_gte=${dateId1}&date_lte=${dateId2}`}>
              격리환자
            </MenuLink>
            <MenuLink to={`/released?date_gte=${dateId1}&date_lte=${dateId2}`}>
              격리해제
            </MenuLink>
            <MenuLink
              to={`/released_acc?date_gte=${dateId1}&date_lte=${dateId2}`}
            >
              누적격리해제
            </MenuLink>
            <MenuLink to={`/death?date_gte=${dateId1}&date_lte=${dateId2}`}>
              사망자
            </MenuLink>
            <MenuLink to={`/death_acc?date_gte=${dateId1}&date_lte=${dateId2}`}>
              누적사망자
            </MenuLink>
          </nav>
        </>
