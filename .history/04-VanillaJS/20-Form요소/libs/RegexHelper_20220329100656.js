...
class RegexHelper {
   ...
   */
  compareToh(origin, compare, msg) {
    this.value(origin, msg);
    this.value(compare, msg);

    const src = document.querySelector(origin).value.trim(); //원본값을 가져온다.
    const dsc = document.querySelector(compare).value.trim(); //비교할 값을 가져온다.
  ...
  phone(selector, msg) {
    ...
    const check1 = /^01(?:0|1|[6-9])(?:\d{3}|\d{4})\d{4}$/; //핸드폰 형식
    const check2 = /^\d{2,3}\d{3,4}\d{4}$/; //집전화 형식
