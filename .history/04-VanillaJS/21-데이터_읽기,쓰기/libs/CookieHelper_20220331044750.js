class CookieHelper {
   ...
   * @param {josn} option - 유효시간, 유효경로, 도메인등을 json으로 묶어서 보낸다
   */
  ...
  setCookie(name, value, options = {}) {
    ...
    //expires 값이 Date클래스의 객체라면 UTCString 타입으로 변환함.(max-age가 있으면 사용 안해도 됨)
    if (options.expires instanceof Date) {
      eptions.expires = options.expires.toUTCString();
    }
  ...
  deletedCookie(name) {
    //max-age값을 0으로 설정하여 name에 대한 쿠키가 즉시 삭제하도록 설정
  ...
  getCookie(name) {
    ...
    //반환할 값이 있다면 decodeing을 수행하고 없다면 undefined를 반환
